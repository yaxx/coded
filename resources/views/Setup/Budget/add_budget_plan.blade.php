         ...
         <div class="portlet-body">
             ...
             <form method="post" action="{{ route('budget_plan.store') }}" name="addBudgetPlan" id="addBudgetPlan">
                    <div class="row">
                           ...
                           <!--  <div class="input-group"> -->
                                ...
                                <select name="agency_id" id="agency_id" class="form-control">
                                   <option value="">-- Select an Agency --</options>
                                    @php $count = $agencies->count(); @endphp
                                    @foreach($agencies as $agency)
                                    <option value="{{ $agency->agency_id }}" {{ old('agency_id') == $agency->agency_id ? "selected" : '' }} agency="{{ $agency->agency_name }}" {{$count==1?'selected':''}}>
                        ...
                        <div class="col-md-6 form-group {{ $errors->has('agency_id') ? ' has-error' : '' }}">
                            <label for="agency_id" class="control-label">Plan Year <span class="required">*</span></label>
                           <!--  <div class="input-group"> -->
                                <select name="budget_year" id="budget_year" class="form-control">
                                   <option value="">-- Select a Plan Name--</options>
                    ...
                    <div class="row">
                        <div class="col-md-4 form-group {{ $errors->has('budget_plan_name') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-4 form-group {{ $errors->has('plan_month') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-4 form-group {{ $errors->has('plan_year') ? ' has-error' : '' }}">
                ...
                function populateBudgets(){
                    ...
                    if(agency_id){
                        $("table tbody").html("<div class='row' align='center'><em>Loading...</em></div>");
                                let data = {"agency_name": agency_name, '_token': _token , 'budget_reference_no' : ref };
                                                ...
                                                //console.log(budgets);
                                                    if(budgets.length > 0){
                                                        ...
                                                        let rows = +(budget.budget_id) + Math.floor(1000 + Math.random() * 9000);
                                                            // console.log(rows);
                                                            let markup = "<tr class='itemRows' row='"+ rows +"'><input type='hidden' name='budget_reference_no[]' value='"+budget.budget_reference_no+"' /><input type='hidden' name='budget_id[]' value='"+budget.budget_id+"' /><td>"+budget.budget_item+"</td><td>"+budget.budget_name+"</td><td>"+budget.prev_total_amount+"</td><td>"+budget.total_amount+"</td><td><input name='amount_requested[]' id='amount_requested_"+rows+"' type='text' row='"+rows+"' class='form-control' onkeyup='return calculateGrandTotal()' value='0.00' required></td><td><input name='budget_plan_item_description[]' id='budget_plan_item_description_"+rows+"' type='text' row='"+rows+"' class='form-control' required ></td><td><button type='button' onclick='' class='btn red btn-outline deleteRow' style='width:100%;'><i class='fa fa-trash'></i></button></td></tr>";
                ...
                function populateBudgetYear(){
                    ...
                    if(agency_id){
                         $('#loading-bar-spinner').show();
                                $.ajax({
                                    method: "POST",
                                    url: "<?php echo route('load_agence_bydget_year') ?>",
                                    data: data,
                                    success: function(data) {
                                        let status = data.status;
                                        let mssg = data.mssg;
                                            if (status) { 
                                                ...
                                                //console.log(budgets);
                                                    if(budgets.length > 0){
                                                        let dropdown = `<select name="budget_year" id="budget_year" class="form-control">
                                   <option value="">-- Select a Plan Year--</options>`;
                                                        $.each(budgets, function(i, budget) {
                                                            dropdown += `<option value="${budget.budget_reference_no}">${budget.budget_name}</option>`;
                                                        });
                                                        dropdown += `</select>`;
                                                        $('#budget_year').html(dropdown)
                                                    }
                                                    else{
                                                        let dropdown = `<select name="budget_year" id="budget_year" class="form-control">
                                   <option value="">-- Select a Plan Year--</options>`;
